...
@section('title', 'Sửa văn bản: ' . $isoSystemDocument->title)
            ...
            <form method="POST" action="{{ route('admin.iso-system-documents.update', $isoSystemDocument) }}" class="admin-form" enctype="multipart/form-data">
                ...
                <div class="admin-form__row">
                    <div class="admin-form__group">
                        ...
                        <input type="text" name="title" value="{{ old('title', $isoSystemDocument->title) }}" required 
                ...
                <div class="admin-form__row">
                    <div class="admin-form__group">
                        ...
                        <textarea name="description" rows="4" 
                                  ...
                                  placeholder="Nhập mô tả văn bản (tùy chọn)">{{ old('description', $isoSystemDocument->description) }}</textarea>
                        @error('description')
                        <div class="admin-form__error">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="admin-form__group">
                        ...
                        <select name="department_id" id="department_id"
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->id }}" {{ old('department_id', $isoSystemDocument->department_id) == $department->id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                @if($isoSystemDocument->file_name)
                <div class="admin-form__row">
                    <div class="admin-form__group">
                        ...
                        <div class="current-file-info">
                            <div class="current-file-info__item">
                                ...
                                <div class="current-file-info__details">
                                    <div class="current-file-info__name">{{ $isoSystemDocument->file_name }}</div>
                                    <div class="current-file-info__meta">{{ $isoSystemDocument->getFormattedFileSize() }} • {{ strtoupper($isoSystemDocument->file_type) }}</div>
                                </div>
                                <a href="{{ route('admin.iso-system-documents.download', $isoSystemDocument) }}" target="_blank" class="current-file-info__download">
                ...
                <div class="admin-form__row">
                    <div class="admin-form__group">
                        <label class="admin-form__label">File tài liệu {{ $isoSystemDocument->file_name ? '(Tùy chọn - để trống nếu không đổi)' : '(Bắt buộc)' }}</label>
                        <div class="admin-file-upload">
                            <input type="file" name="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt"
                                   ...
                                   id="adminFileInput" {{ !$isoSystemDocument->file_name ? 'required' : '' }}>
                ...
                <div class="admin-form__row">
                    <div class="admin-form__group">
                        ...
                        <select name="status" class="admin-form__select @error('status') admin-form__select--error @enderror">
                            <option value="draft" {{ old('status', $isoSystemDocument->status) == 'draft' ? 'selected' : '' }}>Bản nháp</option>
                            <option value="approved" {{ old('status', $isoSystemDocument->status) == 'approved' ? 'selected' : '' }}>Đã phê duyệt</option>
                            <option value="archived" {{ old('status', $isoSystemDocument->status) == 'archived' ? 'selected' : '' }}>Lưu trữ</option>
                        </select>
                        @error('status')
                        <div class="admin-form__error">{{ $message }}</div>
                        @enderror
