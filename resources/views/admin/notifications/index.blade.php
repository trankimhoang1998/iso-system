...
<div class="admin-page">
    <div class="admin-page__header">
        <div class="admin-page__title-section">
            <h1 class="admin-page__title">Quản lý thông báo</h1>
            <p class="admin-page__subtitle">
                Quản lý thông báo hệ thống và thông tin quan trọng
            </p>
        </div>
        ...
        <table class="admin-table">
            <thead class="admin-table__head">
                <tr>
                    ...
                    <th class="admin-table__header admin-table__header--summary">Tiêu đề thông báo</th>
            ...
            <tbody class="admin-table__body" id="notifications-table">
                @forelse($notifications as $notification)
                <tr class="admin-table__row" data-id="{{ $notification->id }}">
                    @if(auth()->user()->role == 0)
                    <td class="admin-table__cell drag-handle" style="cursor: grab; text-align: center;">≡</td>
                    @endif
                    <td class="admin-table__cell admin-table__cell--date">{{ $notification->issue_date->format('d/m/Y') }}</td>
                    <td class="admin-table__cell admin-table__cell--summary">{{ $notification->title }}</td>
                    <td class="admin-table__cell admin-table__cell--document">
                        @if($notification->document_link)
                        <a href="{{ $notification->document_link }}" target="_blank" class="admin-link">
                    ...
                    <td colspan="{{ auth()->user()->role == 0 ? '5' : '4' }}" class="admin-table__empty">
                        <div class="admin-empty-state">
                            <svg class="admin-empty-state__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-5 5v-5zM4 2v16l5-5h6a2 2 0 002-2V2H4z"></path>
                            </svg>
                            <h3 class="admin-empty-state__title">Chưa có thông báo nào</h3>
                            ...
                            @if(in_array(auth()->user()->role, [0, 1]))
                            <a href="{{ route('admin.notifications.create') }}" class="admin-empty-state__btn">
...
<div id="deleteModal" class="admin-modal">
    <div class="admin-modal__content">
        <div class="admin-modal__header">
            <h3 class="admin-modal__title">Xác nhận xóa thông báo</h3>
        ...
        <div class="admin-modal__body">
            <div class="admin-modal__message">
                ...
                <div class="admin-modal__message-content">
                    <p class="admin-modal__message-title">Bạn có chắc chắn muốn xóa thông báo?</p>
                    <p class="admin-modal__message-text">Thông báo <strong id="deleteNotificationName"></strong> sẽ bị xóa vĩnh viễn. Hành động này không thể hoàn tác.</p>
                </div>
            </div>
            ...
            <div class="admin-form__actions">
                ...
                <form method="POST" id="deleteForm" style="display: inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="admin-btn admin-btn--danger">Xóa thông báo</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
function openDeleteModal(notificationId, notificationTitle) {
    document.getElementById('deleteNotificationName').textContent = notificationTitle;
    document.getElementById('deleteForm').action = `/notifications/${notificationId}`;
    ...
    if (tableBody) {
        new Sortable(tableBody, {
            ...
            onEnd: function(evt) {
                ...
                fetch('{{ route("admin.notifications.reorder") }}', {
                    method: 'POST',
                    headers: {
